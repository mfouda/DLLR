...
% save('F:\code\DLLR\mask_all_1d_360.mat','mask_all_1d');
...
U3_index = randperm(lk,floor(lk./2.5));
mask_all_randn1D(U2_index,:,1)=ones(lk./2,lk);
mask_all_randn1D(U3_index,:,2)=ones(floor(lk./2.5),lk);
csize=4;
central = (lk-csize)./2:(lk+csize)./2 -1 ;central=central';
mask_all_randn1D(U2_index,:,3)=ones(lk./2,lk); mask_all_randn1D(central,:,3)=ones(csize,lk);
csize=6;
central = (lk-csize)./2:(lk+csize)./2 -1 ;central=central';
mask_all_randn1D(U3_index,:,4)=ones(floor(lk./2.5),lk); mask_all_randn1D(central,:,4)=ones(csize,lk);
%%
imshow(mask_all_randn1D(:,:,3));
save('F:\code\DLLR\mask_all_randn1D128.mat','mask_all_randn1D');
